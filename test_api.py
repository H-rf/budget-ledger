...
def test_create_transaction_stores_database_row():
    ...
    assert db.get_transaction_by_id(1)==(1, "Salary", 4000.0, "income")

def test_create_transaction_invalid_amount_returns_400_and_stores_nothing():
    response = client.post("/transactions", json={
        "description":"Salary", 
        "amount":-50, 
        "kind":"income"})
    ...
    assert response.status_code == 400
    assert response.json()=={"detail": "amount must be a positive number"}

    assert db.get_all_transactions() == []

def test_create_transaction_invalid_kind_returns_400_and_stores_nothing():
    response = client.post("/transactions", json={
        "description":"Salary", 
        "amount":4000, 
        "kind":"food"})
...
def test_create_transaction_missing_amount_returns_422_and_stores_nothing():
    response = client.post("/transactions", json={
        "description":"Salary", 
        "kind":"income"})
...
def test_get_transaction_by_id_returns_database_row():
    new_id = db.add_transaction("Salary", 4000, "income")
    assert new_id == 1
...
def test_get_transaction_by_id_missing_returns_404():
    response = client.get("/transactions/999")

    assert response.status_code == 404
    assert response.json()=={"detail": "Transaction not found"}

def test_get_transaction_by_id_invalid_id_returns_400():
    response = client.get("/transactions/0")

    assert response.status_code == 400
    assert response.json()=={"detail": "transaction_id must be a positive integer"}

def test_update_transaction_by_id_updates_database_row():
    new_id = db.add_transaction("Salary", 4000, "income")
    assert new_id == 1

    response = client.patch(f"/transactions/{new_id}",
    json={
        "amount":500
    }
)
    assert response.status_code==200
    assert response.json()=={"status": "ok"}
    assert db.get_transaction_by_id(new_id)==(1, "Salary", 500.0, "income")

def test_update_transaction_by_id_missing_returns_404():
    response = client.patch("/transactions/999",
    json={
        "amount":500
    }
)
...
def test_update_transaction_by_id_invalid_id_returns_400():
    response = client.patch("/transactions/0",
    json={
        "amount":500
    }
)
...
def test_update_transaction_by_id_invalid_amount_returns_400_and_keeps_original_row():
    new_id = db.add_transaction("Salary", 4000, "income")
    assert new_id == 1
...
def test_delete_transaction_by_id_deletes_database_row():
    new_id = db.add_transaction("Salary", 4000, "income")
    assert new_id == 1

    response = client.delete(f"/transactions/{new_id}")

    assert response.status_code == 200
    assert response.json()=={"status": "ok"}
    
    assert db.get_transaction_by_id(new_id) is None
    assert db.get_all_transactions()==[]

def test_delete_transaction_by_id_missing_returns_404_and_keeps_existing_rows():
    new_id = db.add_transaction("Salary", 4000, "income")
    assert new_id == 1
...
def test_delete_transaction_by_id_invalid_id_returns_400_and_keeps_existing_rows():
    new_id = db.add_transaction("Salary", 4000, "income")
    assert new_id == 1

    response = client.delete("/transactions/0")
